...
gcs_cred = service_account.Credentials.from_service_account_file("<YOUR_JSON_FILE>")
...
def completeElection():
    ...
    if len(result) == len(ip_list):
        subprocess.run(["gcloud", "dataproc", "jobs", "submit", "spark", "--async", "--cluster=<dataproc-cluster>", "--region=us-west1", "--jar=evote-spark_2.12-1.0.0.jar", "--", "*.csv", "output/"], shell=False, capture_output=True)
